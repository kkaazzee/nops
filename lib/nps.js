...
async function main(input = {}) {
  const interactiveInput = isRunDirectly ? await nopsPrompt(fixtures) : {};
  const options = {
    // Behavior options.
    command: 'pkg_dec',
    dirty: false,
    ...
    filePath: path.join(interactiveInput.downloadDirectory, `${interactiveInput.url.split('/').pop()}`),
...
if (isRunDirectly) {
  ...
  program.
    ...
    usage('[options]').
    option('-d, --dirty', 'Disable deletion downloaded PKG after successful decryption and extraction.').
    option('-v, --verbose', 'Enabled verbose output to stdout and stderr.').
    parse(process.argv);
